...
mod test {
    ...
    fn test_eq_unpadded(input_a: &str, input_b: &str) {
        ...
        assert_eq!(
            input_a == input_b,
            client_key.decrypt_bool(&server_key.eq_unpadded(&s1, &s2))
        );
        assert_eq!(
            input_a == input_a,
            client_key.decrypt_bool(&server_key.eq_unpadded(&s1, &s1))
        );
        assert_eq!(
            input_b == input_a,
            client_key.decrypt_bool(&server_key.eq_unpadded(&s2, &s1))
        );
    }
}
