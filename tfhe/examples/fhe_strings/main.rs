...
fn main() -> io::Result<()> {
    ...
    let command = Command::new("Homomorphic string operations")
        ...
        .arg(
            Arg::new("function")
                ...
                .long("function")
                .required(false)
                .help("Run a specific function (empty/default is to run all)")
                .action(ArgAction::Set),
        );
    ...
    if let (Some(input_string), Some(pattern)) = (
        ...
        if function.is_empty() || function == "ends_with" {
            let now = Instant::now();
            ...
            info!(
                "`ends_with` FHE: {decrypted_ends_with_clear} (took {elapsed:?}) (clear pattern)"
            );
            let now = Instant::now();
        ...
        if function.is_empty() || function == "find" {
            let now = Instant::now();
            ...
            info!(
                "`find` FHE: {decrypted_find_encrypted:?} (took {elapsed:?}) (encrypted pattern)"
            );
            info!("`find` std: {:?}", input_string.find(pattern.as_str()));
        }

        if function.is_empty() || function == "eq_ignore_case" {
            let now = Instant::now();
        ...
        if function.is_empty() || function == "is_empty" {
            let now = Instant::now();
        ...
        if function.is_empty() || function == "len" {
            let now = Instant::now();
        ...
        if function.is_empty() || function == "repeat" {
            let now = Instant::now();
        ...
        if function.is_empty() || function == "replace" {
            let now = Instant::now();
            let replace_clear =
        ...
        if function.is_empty() || function == "replacen" {
            let now = Instant::now();
            ...
            let elapsed = now.elapsed();
            let decrypted_replacen_clear_encrypted =
                client_key.decrypt_str(&replacen_clear_encrypted);
            ...
            let elapsed = now.elapsed();
            let decrypted_replacen_encrypted_clear =
                client_key.decrypt_str(&replacen_encrypted_clear);
            info!(
        ...
        if function.is_empty() || function == "rfind" {
            let now = Instant::now();
            ...
            info!(
                "`rfind` FHE: {decrypted_rfind_encrypted:?} (took {elapsed:?}) (encrypted pattern)"
            );
            info!("`rfind` std: {:?}", input_string.rfind(pattern.as_str()));
        }

        if function.is_empty() || function == "to_lowercase" {
            let now = Instant::now();
        ...
        if function.is_empty() || function == "to_uppercase" {
            let now = Instant::now();
        ...
        if function.is_empty() || function == "le" {
            let now = Instant::now();
        ...
        if function.is_empty() || function == "ge" {
            let now = Instant::now();
        ...
        if function.is_empty() || function == "eq" {
            let now = Instant::now();
        ...
        if function.is_empty() || function == "ne" {
            let now = Instant::now();
        ...
        if function.is_empty() || function == "concat" {
            let now = Instant::now();
        ...
        if function.is_empty() || function == "strip_prefix" {
            let now = Instant::now();
        ...
        if function.is_empty() || function == "strip_suffix" {
            let now = Instant::now();
        ...
        if function.is_empty() || function == "trim" {
            let now = Instant::now();
        ...
        if function.is_empty() || function == "trim_end" {
            let now = Instant::now();
        ...
        if function.is_empty() || function == "trim_start" {
            let now = Instant::now();
        ...
        if function.is_empty() || function == "rsplit" {
            let now = Instant::now();
        ...
        if function.is_empty() || function == "rsplit_once" {
            let now = Instant::now();
        ...
        if function.is_empty() || function == "rsplitn" {
            let now = Instant::now();
            ...
            let now = Instant::now();
            let rsplitn =
                server_key.rsplitn(&encrypted_str, encrypted_n.clone(), &encrypted_pattern);
            let elapsed = now.elapsed();
        ...
        if function.is_empty() || function == "rsplit_terminator" {
            let now = Instant::now();
            ...
            let now = Instant::now();
            let rsplit_terminator =
                server_key.rsplit_terminator(&encrypted_str, &encrypted_pattern);
            let elapsed = now.elapsed();
        ...
        if function.is_empty() || function == "split" {
            let now = Instant::now();
            ...
            info!(
                "`split` FHE: {decrypted_split_encrypted:?} (took {elapsed:?}) (encrypted pattern)"
            );
            info!(
                "`split` std: {:?}",
        ...
        if function.is_empty() || function == "split_ascii_whitespace" {
            let now = Instant::now();
        ...
        if function.is_empty() || function == "split_inclusive" {
            let now = Instant::now();
        ...
        if function.is_empty() || function == "split_terminator" {
            let now = Instant::now();
        ...
        if function.is_empty() || function == "splitn" {
            let now = Instant::now();
